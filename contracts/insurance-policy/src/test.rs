...
fn create_payment_contract(env: &Env) -> Address {
...
fn test_initialize_contract() {
    ...
    let payment_contract = create_payment_contract(&env);
...
fn test_activate_policy() {
    ...
    client.activate_policy(&policy_id, &payment_contract);
...
fn test_invalid_policy_data() {
    ...
    let backfilled_policy_id = client.create_policy(
        &user,
        &location,
        &asset,
        &1_000_000_000i128,
    ...
    let backfilled_policy = client.get_policy(&backfilled_policy_id);
    assert_eq!(backfilled_policy.status, PolicyStatus::Draft);
    assert_eq!(backfilled_policy.start_date, current_time - 86_400);
    assert_eq!(backfilled_policy.end_date, current_time + 31_536_000);
}
