use soroban_sdk::{
    contract, contractimpl, contractmeta, xdr::ToXdr, Address, Env, String, Vec, log
};
use insurance_shared::{
    Policy, PolicyStatus, ContractConfig, InsuranceError, InsuranceEvent,
    Location, Oracle, ParametricTrigger, generate_id,
...
impl InsurancePolicyContract {
    ...
    pub fn initialize(
        ...
        storage::initialize_contract(&env, &config, &payment_contract);
    ...
    pub fn create_policy(
        ...
        let id_seed = (
            ...
            start_date,
            end_date,
            parametric_triggers.clone(),
        )
            .to_xdr(&env);
        let policy_id = generate_id(&env, "POL", &id_seed);
    ...
    pub fn activate_policy(
        ...
        // Only the payment contract can activate a draft policy after collecting premium.
